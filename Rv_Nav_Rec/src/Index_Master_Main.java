...
public class Index_Master_Main {
	...
	public static void main(String[] args) {
		...
		LineIterator it = FileUtils.lineIterator(new File("/home/rv/Desktop/files_to_upload/index_mst.ace"), "UTF-8");
			...
			while (it.hasNext()) // if the file has lines 
     	    {
			   ...
			   tmp_val=it.nextLine();
				...
				Index_Master im = new Index_Master();
				
				im.setIndexCode(Long.parseLong(separated[0]));
				
				if(separated[1]!=null && !separated[1].isEmpty())
				{
					 im.setFincode(Long.parseLong(separated[1]));
				}
				
				if(separated[2]!=null && !separated[2].isEmpty())
				{
					im.setScripcode(separated[2]); 
				}
				
				if(separated[3]!=null && !separated[3].isEmpty())
				{
					im.setIndexName(separated[3]);
				}
